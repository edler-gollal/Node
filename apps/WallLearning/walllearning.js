...
exports = module.exports = function() {
  ...
  Game.prototype = {
    ...
    setup: function() {
      this.player.y = (this.gameHeight / 10) * 8;
      this.player.x = (this.gameWidth / 2) - this.player.width;
  ...
  function Obstacle(game) {
    ...
    this.gapLeftX = parseInt(Math.random() * (this.game.gameWidth - this.gapSize));
    //Avoid free score without moving
    if(game.tick == game.obstacleDelay)
  ...
  Obstacle.prototype = {
    ...
    update: function() {
      ...
      if(this.y > this.game.gameHeight + 50) {
        this.delete();
      }
    },
  ...
  function learning () {
    ...
    var input = game.getNetworkInputs();
    var output = currentNetwork.activate(input);
    game.player.x = parseInt(game.gameWidth * output[0]);
    game.networkOutput = output;
  ...
  function nextNetwork() {
    ...
    if(currentNetworkID == generationSize) {
      ...
      if((currentGeneration[0][0] == 1)&&(game.highScore == 1)){
        currentGeneration = [];
      }
      newGeneration();
    } else {
  ...
  var d = new Date();
  var currentLog = 'log_' + d.getFullYear() + (d.getMonth()+1) + d.getDay() + '_' + d.getHours() + d.getMinutes() + '.txt';
