...
function useDataForLearning(data) {
  var input = [];
  for(var i in data){
    var player = data[i];
    if(i == global.id) {
...
function learning (input) {
  ...
  if((timeNow - time) > 10000) {
    currentGeneration[currentNetworkID][0] = fitness;
    console.log("Genome " + (currentNetworkID+1) + " finished, fitness: " + fitness);
    fitness = 0;
    nextNetwork();
  } else {
...
function controlGameByOutput(output) {
  global.socket.emit('player_register_input', {
    input: 87, //W
    pressed: transformOutputToBoolean(output[0])
  });

  global.socket.emit('player_register_input', {
    input: 65, //A
    pressed: transformOutputToBoolean(output[1])
  });

  global.socket.emit('player_register_input', {
    input: 83, //S
    pressed: transformOutputToBoolean(output[2])
  });

  global.socket.emit('player_register_input', {
    input: 68, //D
    pressed: transformOutputToBoolean(output[3])
  });

  global.socket.emit('player_register_input', {
    input: "mouse",
    pressed: transformOutputToBoolean(output[4])
  });

  var mouseX = global.c.width * output[5];
  var mouseY = global.c.height * output[6];
...
function newNetwork() {
  ...
  var hiddenLayer3 = new Layer(hiddenLayers);
  var hiddenLayer4 = new Layer(hiddenLayers);
  var outputLayer = new Layer(7);
...
function transformOutputToBoolean(value) {
  if(value > 0.5) {
    return true;
  } else {
    return false;
